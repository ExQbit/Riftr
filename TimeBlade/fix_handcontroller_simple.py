...
try:
    ...
    # Wir müssen das Ende der HandController-Klasse finden
    lines = content.split('\n')
    
    # Finde die Zeile mit der letzten schließenden Klammer
    last_brace_line = -1
    brace_count = 0
    in_class = False
    
    for i, line in enumerate(lines):
        if 'public class HandController' in line:
            in_class = True
            brace_count = 0
    ...
    if last_brace_line == -1:
        print("FEHLER: Konnte das Ende der HandController-Klasse nicht finden!")
    else:
        # Füge die Methoden vor der letzten schließenden Klammer ein
        lines.insert(last_brace_line, methods_to_add)
        ...
        print(f"✅ Erfolgreich! Die fehlenden Methoden wurden zu HandController.cs hinzugefügt.")
        print(f"   Position: Vor Zeile {last_brace_line} (Ende der Klasse)")
