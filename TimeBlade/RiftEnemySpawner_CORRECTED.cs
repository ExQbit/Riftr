// KORRIGIERTE VERSION - Ersetze die komplette RiftEnemySpawner.cs mit diesem Code
...
/// KORREKTE IMPLEMENTIERUNG: 7 aktive (sichtbare) + 3 Reserve (unsichtbare) Gegner
/// </summary>
public class RiftEnemySpawner : MonoBehaviour
{
    ...
    [Header("Spawn-Limitierung (KORREKTE WERTE)")]
    [SerializeField] public int maxActiveEnemiesInQueue = 7; // Maximale SICHTBARE Gegner die angreifen können
    [SerializeField] public int maxReserveQueueEnemies = 3; // Maximale UNSICHTBARE Reserve-Gegner die NICHT angreifen
    ...
    public void StartSpawning(RiftTimeSystem.RiftType riftType)
    {
        ...
        Debug.Log($"[EnemySpawner] Spawn-Limits: {maxActiveEnemiesInQueue} aktive (sichtbar) + {maxReserveQueueEnemies} reserve (unsichtbar) = {maxActiveEnemiesInQueue + maxReserveQueueEnemies} total");
    ...
    private void SpawnEnemy()
    {
        ...
        if (!CanSpawnEnemy())
        {
            Debug.Log("[EnemySpawner] Spawn-Limit erreicht, warte auf freien Platz");
            return;
        }
    ...
    private bool CanSpawnEnemy(int additionalEnemies = 1)
    {
        ...
        // Hole aktuelle Anzahl der Gegner
        int activeCount = GetActiveEnemyCount();
        int reserveCount = GetReserveEnemyCount();
        int totalCount = activeCount + reserveCount;
        int maxTotal = maxActiveEnemiesInQueue + maxReserveQueueEnemies;
        
        // Prüfe Gesamt-Maximum (aktive + reserve)
        if ((totalCount + additionalEnemies) > maxTotal)
        {
            Debug.Log($"[EnemySpawner] Spawn verweigert: Total-Limit erreicht ({totalCount + additionalEnemies}/{maxTotal})");
            Debug.Log($"[EnemySpawner] Details: {activeCount} aktive (sichtbar), {reserveCount} reserve (unsichtbar)");
            return false;
        }
        
        Debug.Log($"[EnemySpawner] Spawn erlaubt: Aktive={activeCount}/{maxActiveEnemiesInQueue}, Reserve={reserveCount}/{maxReserveQueueEnemies}, Total={totalCount}/{maxTotal}");
    ...
    private int GetActiveEnemyCount()
    {
        ...
        // Aktive Gegner sind die ersten maxActiveEnemiesInQueue in der Queue
        var queue = EnemyFocusSystem.Instance.GetEnemyQueue();
        return Mathf.Min(queue.Count, maxActiveEnemiesInQueue);
    }
    
    /// <summary>
    /// Zählt Reserve-Gegner (unsichtbar, greifen nicht an)
    /// </summary>
    private int GetReserveEnemyCount()
    {
        if (EnemyFocusSystem.Instance == null) return 0;
        
        var queue = EnemyFocusSystem.Instance.GetEnemyQueue();
        int totalCount = queue.Count;
        int activeCount = Mathf.Min(totalCount, maxActiveEnemiesInQueue);
        
        // Reserve sind alle Gegner über dem aktiven Limit
        return Mathf.Max(0, totalCount - activeCount);
    }
    ...
    private int GetTotalEnemyCount()
    {
        ...
        // Zähle alle Gegner in der Queue
        return EnemyFocusSystem.Instance.GetEnemyQueue().Count;
    }
