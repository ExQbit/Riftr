...
// ═══════════════════════════════════════════
// ─── Recovery: portfolio_history + cost_basis from PITR window ───
// ═══════════════════════════════════════════
//
// Use-Case: nach versehentlichem Loeschen via cleanupBetaMarketplace
// (vor dem Bug-Fix). Firestore behaelt 1h Default-Recovery-Window auch
// ohne PITR enabled. Wenn Cleanup innerhalb des Fensters lief, koennen
// wir die alten Doc-Versionen via readTime-Query holen + restoren.
//
// USAGE:
//   curl -X POST -H "Authorization: Bearer $ADMIN_TRIGGER_SECRET" \
//     "https://us-central1-riftr-10527.cloudfunctions.net/recoverPortfolioFromPitr?readTime=2026-04-29T12:52:15Z"
//
// readTime-Query-Param: ISO-Timestamp innerhalb des Fensters. Faustregel:
// `earliestVersionTime` aus `firebase firestore:databases:get` plus 1 Min.

exports.recoverPortfolioFromPitr = onRequest(
  {
    timeoutSeconds: 300,
    memory: "512MiB",
    region: "us-central1",
  ...
  async (req, res) => {
    ...
    const readTimeStr = req.query.readTime || req.body?.readTime;
    if (!readTimeStr) {
      res.status(400).json({
        error: "Missing readTime query param. Use ISO-Timestamp like 2026-04-29T12:52:15Z",
      });
      return;
    }

    const readTimestamp = admin.firestore.Timestamp.fromDate(new Date(readTimeStr));
    ...
    const summary = {
      ...
      users_checked: 0,
      portfolio_history_recovered: 0,
      portfolio_history_not_in_pitr: 0,
      cost_basis_recovered: 0,
      cost_basis_not_in_pitr: 0,
      errors: [],
    };
    ...
    for (const userDoc of userDocs) {
      ...
      try {
        ...
        const phPitrSnap = await phRef.get({ readTime: readTimestamp });
        if (phPitrSnap.exists) {
          await phRef.set(phPitrSnap.data());
          summary.portfolio_history_recovered++;
          console.log(`Restored portfolio_history for ${uid}`);
        } else {
          summary.portfolio_history_not_in_pitr++;
        }
      } catch (e) {
        summary.errors.push(`portfolio_history ${uid}: ${e.message}`);
      }

      // ── cost_basis ──
      try {
        const cbRef = userDoc.collection("data").doc("cost_basis");
        const cbPitrSnap = await cbRef.get({ readTime: readTimestamp });
        if (cbPitrSnap.exists) {
          await cbRef.set(cbPitrSnap.data());
          summary.cost_basis_recovered++;
          console.log(`Restored cost_basis for ${uid}`);
        } else {
          summary.cost_basis_not_in_pitr++;
        }
      } catch (e) {
        summary.errors.push(`cost_basis ${uid}: ${e.message}`);
      }
    }

    console.log(`recoverPortfolioFromPitr: ${JSON.stringify(summary)}`);
    res.json({ success: true, ...summary });
  },
);
